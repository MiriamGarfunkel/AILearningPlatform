...
export const generateLesson = async (
  ...
  const userId = authed.user?._id || body_user_id;
  ...
  try {
    const saved = await sessions.saveLesson({
      userId,
...
export const getUserHistory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const history = await sessions.getUserHistory(subject);
...
export const getAllHistory = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { rows, total } = await sessions.getAllHistory(page, limit);
